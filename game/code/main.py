...
class Game(object):
    ...
    def add_player(self):
        ...
        # starting items
        self.player.inventory.add_item(objects.item.crossbow)
        self.player.inventory.add_item(objects.item.holy_cross)
        self.player.inventory.add_item(objects.item.holy_water)
        self.player.inventory.add_item(objects.item.flammable_oil)
        # starting journal
    ...
    def _choice_hook(self, choice_string, decision):
        ...
        inject_choices = [interactions.choices.ChoiceEnableDebugMode()]
        # list of debugging choices
        debug_choices = [
            interactions.choices.ChoiceNavigate('level1', key='level1',
                                                level='level_1', room='entrance_hall', hidden=True),
            interactions.choices.ChoiceNavigate('level2', key='level2',
        ...
        if self.is_flag_value('DEBUG', True):
            ...
            if choice_string.startswith('tele'):
                ...
                if len(parts) > 2:
                    tele = interactions.choices.ChoiceNavigate(*parts, key='tele', hidden=True)
